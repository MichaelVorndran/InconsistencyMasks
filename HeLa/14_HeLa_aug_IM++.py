...
with tf.device('/gpu:0'):
    ...
    for runid in range(1,4):
        ...
        if train_new_evalnet == True:
            ...
            main_output_evalnet_im_dir_train = os.path.join(paths.HELA_BASE_DIR, 'evalnet_aug_im', f'run_{runid}', 'train')
            main_output_evalnet_im_dir_val = os.path.join(paths.HELA_BASE_DIR, 'evalnet_aug_im', f'run_{runid}', 'val')
            ...
            for i in range(0,5):
                ...
                modelname_evalnet_im = f'HELA_evalnet_miou_aug_im_{runid}_{i}'
        ...
        for n in range(2,3):
            ...
            for gen in range(0,5):
                ...
                for j in range(1,n+1):
                    best_evalnet_filepaths_h5.append(os.path.join(paths.HELA_MODEL_DIR, f'HELA_evalnet_miou_aug_im_{runid}_topK_{j}.h5'))
                ...
                for imagename in tqdm(os.listdir(train_unlabeled_pseudo_label_dir_im_bf_images)):
                    shutil.copy(os.path.join(train_unlabeled_pseudo_label_dir_im_bf_images, imagename), os.path.join(train_unlabeled_pseudo_label_dir_im_plus_plus_bf_images, imagename))
                    shutil.copy(os.path.join(train_unlabeled_pseudo_label_dir_im_alive_masks, imagename), os.path.join(train_unlabeled_pseudo_label_dir_im_plus_plus_alive_masks, imagename))
                    shutil.copy(os.path.join(train_unlabeled_pseudo_label_dir_im_dead_masks, imagename), os.path.join(train_unlabeled_pseudo_label_dir_im_plus_plus_dead_masks, imagename))
                    shutil.copy(os.path.join(train_unlabeled_pseudo_label_dir_im_pos_masks, imagename), os.path.join(train_unlabeled_pseudo_label_dir_im_plus_plus_pos_masks, imagename))

                for imagename in tqdm(os.listdir(paths.HELA_TRAIN_LABELED_AUG_BRIGHTFIELD_DIR)):
                    shutil.copy(os.path.join(paths.HELA_TRAIN_LABELED_AUG_BRIGHTFIELD_DIR, imagename), os.path.join(train_unlabeled_pseudo_label_dir_im_plus_plus_bf_images, imagename))
                    shutil.copy(os.path.join(paths.HELA_TRAIN_LABELED_AUG_ALIVE_DIR, imagename), os.path.join(train_unlabeled_pseudo_label_dir_im_plus_plus_alive_masks, imagename))
                    shutil.copy(os.path.join(paths.HELA_TRAIN_LABELED_AUG_DEAD_DIR, imagename), os.path.join(train_unlabeled_pseudo_label_dir_im_plus_plus_dead_masks, imagename))
                    shutil.copy(os.path.join(paths.HELA_TRAIN_LABELED_AUG_MOD_POS_DIR, imagename), os.path.join(train_unlabeled_pseudo_label_dir_im_plus_plus_pos_masks, imagename))
