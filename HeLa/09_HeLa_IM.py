...
from functions import train_hela, create_pseudo_labels_im_hela
...
FILTER_INCONSISTENT_CLASS_PRED = config['HELA']['FILTER_INCONSISTENT_CLASS_PRED'].lower() == 'true'
        ...
        for n in range(2,5):
            ...
            for gen in range(0,5):
                ...
                if FILTER_INCONSISTENT_CLASS_PRED:
                    modelname = f'HELA_{approach}_{runid}_n{n}_gen{gen}_e{EK}_d{DK}_bi_{BI}_bo_{BO}_filtered'
                else:
                ...
                val_mean_im_size = create_pseudo_labels_im_hela(best_models, IMAGE_HEIGHT, IMAGE_WIDTH, IMAGE_CHANNELS, paths.HELA_VAL_BRIGHTFIELD_DIR, val_pseudo_label_dir, EK, DK, BI, BO)
                test_mean_im_size = create_pseudo_labels_im_hela(best_models, IMAGE_HEIGHT, IMAGE_WIDTH, IMAGE_CHANNELS, paths.HELA_TEST_BRIGHTFIELD_DIR, test_pseudo_label_dir, EK, DK, BI, BO)
                unlabeled_mean_im_size = create_pseudo_labels_im_hela(best_models, IMAGE_HEIGHT, IMAGE_WIDTH, IMAGE_CHANNELS, paths.HELA_TRAIN_UNLABELED_BRIGHTFIELD_DIR, train_unlabeled_pseudo_label_dir, EK, DK, BI, BO)
