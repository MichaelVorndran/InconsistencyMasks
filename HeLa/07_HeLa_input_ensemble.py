...
from functions import create_pseudo_labels_input_ensemble_hela, train_hela
        ...
        for n in range(3,8,2):
            ...
            for gen in range(0,5):
                ...
                if gen == 0:
                    ...
                    best_model_filepath_h5 = os.path.join(paths.HELA_MODEL_DIR, f'HELA_{approach}_{runid}_n{n}_gen{gen-1}_topK_1.h5')
                
                best_model = tf.keras.models.load_model(best_model_filepath_h5)
                
                create_pseudo_labels_input_ensemble_hela(best_model, paths.HELA_VAL_BRIGHTFIELD_DIR, val_pseudo_label_dir, IMAGE_HEIGHT, IMAGE_WIDTH, IMAGE_CHANNELS, n)
                create_pseudo_labels_input_ensemble_hela(best_model, paths.HELA_TEST_BRIGHTFIELD_DIR, test_pseudo_label_dir, IMAGE_HEIGHT, IMAGE_WIDTH, IMAGE_CHANNELS, n)
                create_pseudo_labels_input_ensemble_hela(best_model, paths.HELA_TRAIN_UNLABELED_BRIGHTFIELD_DIR, train_unlabeled_pseudo_label_dir, IMAGE_HEIGHT, IMAGE_WIDTH, IMAGE_CHANNELS, n)
