...
with tf.device('/gpu:0'):
    ...
    for runid in range(1,4):
        ...
        if train_new_evalnet == True:
            ...
            main_output_evalnet_im_dir_train = os.path.join(paths.SUIM_BASE_DIR, 'evalnet_aug_im', f'run_{runid}', 'train')
            main_output_evalnet_im_dir_val = os.path.join(paths.SUIM_BASE_DIR, 'evalnet_aug_im', f'run_{runid}', 'val')
            ...
            create_training_data_evalnet_miou_im_multiclass(subset_models,
                                                         ...
                                                         NUM_CLASSES,
                                                         paths.SUIM_TRAIN_LABELED_IMAGES_DIR, 
            ...
            for i in range(0,5):
                ...
                modelname_evalnet_im = f'SUIM_evalnet_miou_aug_im_{runid}_{i}'
                model_filepath_h5 = os.path.join(paths.SUIM_MODEL_DIR , f'{modelname_evalnet_im}.h5')
            ...
            with open(os.path.join(paths.SUIM_CSV_DIR, f'results_{modelname_evalnet_im}.csv'), 'w', encoding='utf-8', newline='') as f:
        ...
        for n in range(2,3):
            ...
            for gen in range(0,5):
                ...
                for j in range(1,n+1):
                    best_evalnet_filepaths_h5.append(os.path.join(paths.SUIM_MODEL_DIR, f'SUIM_evalnet_miou_aug_im_{runid}_topK_{j}.h5'))
                ...
                for imagename in tqdm(os.listdir(paths.SUIM_TRAIN_LABELED_AUG_IMAGES_DIR)):
                    shutil.copy(os.path.join(paths.SUIM_TRAIN_LABELED_AUG_IMAGES_DIR, imagename), os.path.join(train_unlabeled_pseudo_label_dir_im_plus_plus_images, imagename))
                    shutil.copy(os.path.join(paths.SUIM_TRAIN_LABELED_AUG_MASKS_DIR, imagename), os.path.join(train_unlabeled_pseudo_label_dir_im_plus_plus_masks, imagename))
