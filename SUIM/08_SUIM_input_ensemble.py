    ...
    for runid in range(1,4):
        ...
        for n in range(3,8,2):
            ...
            for gen in range(0,5):
                ...
                if gen == 0:
                    best_model_filepath_h5 = os.path.join(paths.SUIM_MODEL_DIR, f'SUIM_subset_{runid}_topK_1.h5')
                else:
                    best_model_filepath_h5 = os.path.join(paths.SUIM_MODEL_DIR, f'SUIM_{approach}_{runid}_n{n}_gen{gen-1}_topK_1.h5')
                
                best_model = tf.keras.models.load_model(best_model_filepath_h5)
                
                create_pseudo_labels_input_ensemble_multiclass(best_model, paths.SUIM_VAL_IMAGES_DIR, val_pseudo_label_dir, IMAGE_HEIGHT, IMAGE_WIDTH, IMAGE_CHANNELS, n, True)
                create_pseudo_labels_input_ensemble_multiclass(best_model, paths.SUIM_TEST_IMAGES_DIR, test_pseudo_label_dir, IMAGE_HEIGHT, IMAGE_WIDTH, IMAGE_CHANNELS, n, True)
                create_pseudo_labels_input_ensemble_multiclass(best_model, paths.SUIM_TRAIN_UNLABELED_IMAGES_DIR, train_unlabeled_pseudo_label_dir, IMAGE_HEIGHT, IMAGE_WIDTH, IMAGE_CHANNELS, n, True)
