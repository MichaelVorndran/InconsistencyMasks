...
with tf.device('/gpu:0'):
    ...
    for runid in range(1,4):
        ...
        if train_new_evalnet == True:
            ...
            main_output_evalnet_im_dir_train = os.path.join(paths.ISIC_2018_BASE_DIR, 'evalnet_im', f'run_{runid}', 'train')
            main_output_evalnet_im_dir_val = os.path.join(paths.ISIC_2018_BASE_DIR, 'evalnet_im', f'run_{runid}', 'val')
            ...
            for i in range(0,5):
                modelname_evalnet_im = f'ISIC_2018_evalnet_im_{runid}_{i}'
        ...
        for n in range(2,5):
            ...
            for gen in range(0,5):
                ...
                modelname_last_gen = f'ISIC_2018_{approach}_{runid}_n{n}_gen{gen-1}_e{EK}_d{DK}_bi_{BI}_bo_{BO}'
                ...
                train_unlabeled_pseudo_label_dir_im_last_gen = os.path.join(paths.ISIC_2018_BASE_DIR, 'train_unlabeled_predictions', approach, 'temp', modelname_last_gen)
                train_unlabeled_pseudo_label_dir_im_last_gen_images = os.path.join(train_unlabeled_pseudo_label_dir_im_last_gen, 'images')
                train_unlabeled_pseudo_label_dir_im_last_gen_masks = os.path.join(train_unlabeled_pseudo_label_dir_im_last_gen, 'masks')
                ...
                for j in range(1,n+1):
                    best_evalnet_filepaths_h5.append(os.path.join(paths.ISIC_2018_MODEL_DIR, f'ISIC_2018_evalnet_im_{runid}_topK_{j}.h5'))
                ...
                for imagename in tqdm(os.listdir(paths.ISIC_2018_TRAIN_LABELED_IMAGES_DIR)):
                    shutil.copy(os.path.join(paths.ISIC_2018_TRAIN_LABELED_IMAGES_DIR, imagename), os.path.join(train_unlabeled_pseudo_label_dir_im_plus_plus_images, imagename))
                    shutil.copy(os.path.join(paths.ISIC_2018_TRAIN_LABELED_MASKS_DIR, imagename), os.path.join(train_unlabeled_pseudo_label_dir_im_plus_plus_masks, imagename))
