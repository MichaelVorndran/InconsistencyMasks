...
THRESHOLD = float(config['CITYSCAPES']['MAX_THRESHOLD'])
...
with tf.device('/gpu:0'):
    ...
    for runid in range(1,4):
        ...
        if train_new_evalnet == True:
            ...
            main_output_evalnet_dir_train = os.path.join(paths.CITYSCAPES_BASE_DIR, 'evalnet_miou_ensemble', f'run_{runid}', 'train')
            main_output_evalnet_dir_val = os.path.join(paths.CITYSCAPES_BASE_DIR, 'evalnet_miou_ensemble', f'run_{runid}', 'val')

            model_i = 0
            for modelname in os.listdir(paths.CITYSCAPES_MODEL_DIR):
            
                if f'CITYSCAPES_subset_{runid}' in modelname:        
            
                    model_filepath_h5 = os.path.join(paths.CITYSCAPES_MODEL_DIR, modelname)
            
                    model = tf.keras.models.load_model(model_filepath_h5)
                    create_training_data_evalnet_miou_multiclass(model, IMAGE_HEIGHT, IMAGE_WIDTH, IMAGE_CHANNELS, NUM_CLASSES, paths.CITYSCAPES_TRAIN_LABELED_IMAGES_DIR, paths.CITYSCAPES_TRAIN_LABELED_MASKS_DIR, main_output_evalnet_dir_train, model_i)
                    
                    if model_i < 3:
                        create_training_data_evalnet_miou_multiclass(model, IMAGE_HEIGHT, IMAGE_WIDTH, IMAGE_CHANNELS, NUM_CLASSES, paths.CITYSCAPES_VAL_IMAGES_DIR, paths.CITYSCAPES_VAL_MASKS_DIR, main_output_evalnet_dir_val, model_i)
            ...
            for i in range(0,5):
                ...
                modelname_evalnet = f'CITYSCAPES_evalnet_miou_{runid}_{i}'
                model_filepath_h5 = os.path.join(paths.CITYSCAPES_MODEL_DIR , f'{modelname_evalnet}.h5')
            ...
            with open(os.path.join(paths.CITYSCAPES_CSV_DIR, f'results_{modelname_evalnet}.csv'), 'w', encoding='utf-8', newline='') as f:
        ...
        for n in range(2,5):
            ...
            for gen in range(0,5):
                ...
                modelname = f'CITYSCAPES_segnet_{runid}_n{n}_gen{gen}'
                modelname_last_gen = f'CITYSCAPES_segnet_{runid}_n{n}_gen{gen-1}'

                val_n_pred_dir = os.path.join(paths.CITYSCAPES_BASE_DIR, 'val_predictions', 'segnet', modelname)
                test_n_pred_dir = os.path.join(paths.CITYSCAPES_BASE_DIR, 'test_predictions', 'segnet', modelname)
                train_unlabeled_pseudo_label_dir = os.path.join(paths.CITYSCAPES_BASE_DIR, 'train_unlabeled_predictions', 'segnet', modelname)
                train_unlabeled_pseudo_label_dir_last_gen = os.path.join(paths.CITYSCAPES_BASE_DIR, 'train_unlabeled_predictions', 'segnet', modelname_last_gen)
                ...
                for j in range(1,n+1):
                    best_model_filepaths_h5.append(os.path.join(paths.CITYSCAPES_MODEL_DIR, f'CITYSCAPES_evalnet_miou_{runid}_topK_{j}.h5'))
                ...
                if gen == 0:
                    ...
                    for j in range(0,10):
                        mask_dirs.append(os.path.join(paths.CITYSCAPES_BASE_DIR, 'train_unlabeled_predictions', 'subset', f'CITYSCAPES_subset_{runid}_{j}'))
                    ...
                    for j in range(0,5):
                        mask_dirs.append(os.path.join(paths.CITYSCAPES_BASE_DIR, 'train_unlabeled_predictions', 'segnet', f'CITYSCAPES_segnet_{runid}_n{n}_gen{gen-1}_{j}'))
                ...
                for imagename in tqdm(os.listdir(paths.CITYSCAPES_TRAIN_LABELED_IMAGES_DIR)):
                    ...
                    shutil.copy(os.path.join(paths.CITYSCAPES_TRAIN_LABELED_MASKS_DIR, imagename), os.path.join(train_unlabeled_pseudo_label_dir_masks, imagename))

                num_imgs = len(os.listdir(paths.CITYSCAPES_TRAIN_FULL_IMAGES_DIR))
                ...
                for i in range(0,5):
                    ...
                    val_pred_dir = os.path.join(paths.CITYSCAPES_BASE_DIR, 'val_predictions', 'segnet', modelname_i)
                    test_pred_dir = os.path.join(paths.CITYSCAPES_BASE_DIR, 'test_predictions', 'segnet', modelname_i)
                    train_unlabeled_pred_dir = os.path.join(paths.CITYSCAPES_BASE_DIR, 'train_unlabeled_predictions', 'segnet', modelname_i)
