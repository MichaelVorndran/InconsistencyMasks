        ...
        for n in range(2,5):
            ...
            for gen in range(0,5):
                ...
                if gen == 0:
                    ...
                    for j in range(1,n+1):
                        best_model_filepaths_h5.append(os.path.join(paths.CITYSCAPES_MODEL_DIR, f'CITYSCAPES_{approach}_{runid}_n{n}_gen{gen-1}_topK_{j}.h5'))
                
                for best_model_filepath_h5 in best_model_filepaths_h5:
                    best_model = tf.keras.models.load_model(best_model_filepath_h5)
                    best_models.append(best_model)
                
                create_pseudo_labels_model_ensemble_multiclass(best_models, paths.CITYSCAPES_VAL_IMAGES_DIR, val_pseudo_label_dir, IMAGE_HEIGHT, IMAGE_WIDTH, IMAGE_CHANNELS, True)
                create_pseudo_labels_model_ensemble_multiclass(best_models, paths.CITYSCAPES_TEST_IMAGES_DIR, test_pseudo_label_dir, IMAGE_HEIGHT, IMAGE_WIDTH, IMAGE_CHANNELS, True)
                create_pseudo_labels_model_ensemble_multiclass(best_models, paths.CITYSCAPES_TRAIN_UNLABELED_IMAGES_DIR, train_unlabeled_pseudo_label_dir, IMAGE_HEIGHT, IMAGE_WIDTH, IMAGE_CHANNELS, True)
