...
from functions import train_multiclass, create_pseudo_labels_im_multiclass, create_augment_images_and_masks_multiclass, ignore_im_categorical_crossentropy, ignore_im_dice_loss_multiclass
...
FILTER_INCONSISTENT_CLASS_PRED = config['CITYSCAPES']['FILTER_INCONSISTENT_CLASS_PRED'].lower() == 'true'
        ...
        for n in range(2,5):
            ...
            for gen in range(0,5):
                ...
                if FILTER_INCONSISTENT_CLASS_PRED:
                    modelname = f'CITYSCAPES_{approach}_{runid}_n{n}_gen{gen}_e{EK}_d{DK}_bi_{BI}_bo_{BO}_filtered'
                else:
                ...
                if gen == 0:
                    ...
                    for j in range(1,n+1):
                        if FILTER_INCONSISTENT_CLASS_PRED:
                            modelname_pred_im = f'CITYSCAPES_{approach}_{runid}_n{n}_gen{gen-1}_e{EK}_d{DK}_bi_{BI}_bo_{BO}_filtered'
                        else:
                            modelname_pred_im = f'CITYSCAPES_{approach}_{runid}_n{n}_gen{gen-1}_e{EK}_d{DK}_bi_{BI}_bo_{BO}'

                        best_model_filepaths_h5.append(os.path.join(paths.CITYSCAPES_MODEL_DIR, f'{modelname_pred_im}_topK_{j}.h5'))
                
                for best_model_filepath_h5 in best_model_filepaths_h5:
                    best_model = tf.keras.models.load_model(best_model_filepath_h5)
                    best_models.append(best_model)
                
                val_mean_im_size = create_pseudo_labels_im_multiclass(best_models, IMAGE_HEIGHT, IMAGE_WIDTH, IMAGE_CHANNELS, paths.CITYSCAPES_VAL_IMAGES_DIR, val_pseudo_label_dir_im, True, EK, DK, BI, BO, FILTER_INCONSISTENT_CLASS_PRED)
                test_mean_im_size = create_pseudo_labels_im_multiclass(best_models, IMAGE_HEIGHT, IMAGE_WIDTH, IMAGE_CHANNELS, paths.CITYSCAPES_TEST_IMAGES_DIR, test_pseudo_label_dir_im, True, EK, DK, BI, BO, FILTER_INCONSISTENT_CLASS_PRED)
                unlabeled_mean_im_size = create_pseudo_labels_im_multiclass(best_models, IMAGE_HEIGHT, IMAGE_WIDTH, IMAGE_CHANNELS, paths.CITYSCAPES_TRAIN_UNLABELED_IMAGES_DIR, train_unlabeled_pseudo_label_dir_im, True, EK, DK, BI, BO, FILTER_INCONSISTENT_CLASS_PRED)
                ...
                for imagename in tqdm(os.listdir(paths.CITYSCAPES_TRAIN_LABELED_AUG_IMAGES_DIR)):
                    shutil.copy(os.path.join(paths.CITYSCAPES_TRAIN_LABELED_AUG_IMAGES_DIR, imagename), os.path.join(train_unlabeled_pseudo_label_dir_im_plus_images, imagename))
                    shutil.copy(os.path.join(paths.CITYSCAPES_TRAIN_LABELED_AUG_MASKS_DIR, imagename), os.path.join(train_unlabeled_pseudo_label_dir_im_plus_masks, imagename))
