...
with tf.device('/gpu:0'):
    ...
    for runid in range(1,4):
        ...
        if train_new_evalnet == True:
            ...
            main_output_evalnet_ibas_dir_train = os.path.join(paths.CITYSCAPES_BASE_DIR, 'evalnet_aug_ibas', f'run_{runid}', 'train')
            main_output_evalnet_ibas_dir_val = os.path.join(paths.CITYSCAPES_BASE_DIR, 'evalnet_aug_ibas', f'run_{runid}', 'val')
            ...
            for i in range(0,5):
                ...
                modelname_evalnet_ibas = f'CITYSCAPES_evalnet_miou_aug_ibas_{runid}_{i}'
        ...
        for n in range(2,3):
            ...
            for gen in range(0,5):
                ...
                for j in range(1,n+1):
                    best_evalnet_filepaths_h5.append(os.path.join(paths.CITYSCAPES_MODEL_DIR, f'CITYSCAPES_evalnet_miou_aug_ibas_{runid}_topK_{j}.h5'))
                ...
                for imagename in tqdm(os.listdir(train_unlabeled_pseudo_label_dir_ibas_images)):
                    shutil.copy(os.path.join(train_unlabeled_pseudo_label_dir_ibas_images, imagename), os.path.join(train_unlabeled_pseudo_label_dir_ibas_plus_plus_images, imagename))
                    shutil.copy(os.path.join(train_unlabeled_pseudo_label_dir_ibas_masks, imagename), os.path.join(train_unlabeled_pseudo_label_dir_ibas_plus_plus_masks, imagename))

                for imagename in tqdm(os.listdir(paths.CITYSCAPES_TRAIN_LABELED_AUG_IMAGES_DIR)):
                    shutil.copy(os.path.join(paths.CITYSCAPES_TRAIN_LABELED_AUG_IMAGES_DIR, imagename), os.path.join(train_unlabeled_pseudo_label_dir_ibas_plus_plus_images, imagename))
                    shutil.copy(os.path.join(paths.CITYSCAPES_TRAIN_LABELED_AUG_MASKS_DIR, imagename), os.path.join(train_unlabeled_pseudo_label_dir_ibas_plus_plus_masks, imagename))
